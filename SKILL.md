...
- `tag_collect`：用户想用复选类目/运营标签批量生成候选商品表、验证字段编号/导出流程，或要求打开“标签选品 Web 工作台/筛选测试工作台”。Web 默认使用真实数据模式，通过 Playwright/RPA 打开 1688 搜索页和商品详情页；如遇登录或验证，需要用户在弹出的浏览器中扫码处理。`开发样例`/`sample_verified` 只用于本地开发验证，不代表真实 1688 数据可信。
