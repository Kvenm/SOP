...
- **标签采集工作台**：用复选类目、运营标签和样例数据验证筛选、字段编号、导出与人工复核流程
...
打开 `http://127.0.0.1:8765` 后，可复选类目标签和运营筛选标签，运行采集并下载 XLSX/CSV。页面默认勾选“样例数据”，用于验证筛选、字段编号、结果表格、P0/P1 高潜核验队列、样例详情核验和导出链路，不调用 1688 接口。切换为真实采集时需要先配置 AK；真实详情页核验/RPA 登录尚未接入，导出后仍需人工复核，特别是运费、品退率、发货率、发货时效、一件代发等关键字段。Web 第一版不是绕过 1688 风控的爬虫。
