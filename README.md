# SOP（1688 标签选品与店铺运营工具）

SOP 是一个面向 1688 选品、字段核验、导出复核和后续铺货 dry-run 的本地工具项目，基于 `1688-shopkeeper` 能力扩展。当前重点是“按标签选品采集 + Excel 导出 + 样例详情核验”的可测试 MVP。

详细功能、运行方式、注意事项和当前边界见 [中文项目说明](./docs/SOP-project-guide-cn.md)。
...
- **标签采集工作台**：用复选类目、运营标签和真实 1688 页面数据验证筛选、字段编号、导出与人工复核流程
- **经营问答**：定价策略、运费模板、选品避坑、新店起步等常见问题都能问

## 安装

在 Claw 对话框里直接输入下面这段话，让 OpenClaw 帮你自动安装。上传到你的 GitHub 后，把地址替换为你的 `SOP` 仓库地址：

```text
请帮我安装这个 skill：https://github.com/<你的 GitHub 用户名>/SOP.git
```
...
| `tag_collect` | 标签选品采集 / Web 筛选测试工作台 | `python3 cli.py tag_collect --serve --port 8765` |
...
打开 `http://127.0.0.1:8765` 后，可复选类目标签和运营筛选标签，运行采集并下载 XLSX/CSV。页面默认走真实数据模式，通过 Playwright/RPA 打开 1688 搜索页采集候选商品；如果账号登录不上或一直触发验证，可在“1688 页面 URL”中粘贴浏览器里能打开的 1688 搜索页/商品详情页链接，先测试公开页面真实数据解析。点击“真实详情核验”会进入真实商品详情页提取运费、品退率、发货率等字段；提取不到的字段会标记失败或 `partial_verified`，不会用样例数据补齐。`开发样例` 仅用于本地开发验证。
