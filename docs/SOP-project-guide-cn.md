...
- 标签会先拆成搜索词、1688 页面原生筛选、后置指标区间筛选和系统规则；原生筛选必须由 RPA 在 1688 页面尝试点击/勾选。
- 好评率、品退率、发货率、评论数、复购率、近30天销量、代发订单量已做区间标签，读不到的字段不猜值。
...
- 三级类目路径复选。
- 运营筛选标签复选。
- 1688 原生筛选执行记录：`clicked`、`not_found`、`click_failed`、`not_applicable`。
- 指标区间筛选：例如 `好评率>=90%`、`品退率5%-10%`、`发货率90%-95%`、`评论数30-99`。
...
Web 工作台
-> 选择类目和标签
-> 标签拆分为搜索词、1688原生筛选、后置指标区间和系统规则
-> 点击真实采集
-> Playwright 打开 1688 搜索页
-> RPA 尝试点击/勾选 1688 原生筛选项
-> 从真实页面提取候选商品
-> 去重、评分、标记风险
-> 进入详情核验队列
-> 导出表格
...
样例模式导出的每行都会写入 `数据模式=开发样例` 和 `数据真实性说明=开发样例：未访问1688，禁止作为真实选品/铺货依据`，并额外生成 `样例说明` 工作表。样例行不会标记为普通 `系统预通过`，只会显示 `样例预通过` 或 `样例不建议`。
...
XLSX 真实采集默认包含 6 个工作表；开发样例或混有样例行时，会额外包含 `样例说明` 工作表：
...
正式验收时必须检查 `数据模式`、`数据真实性说明`、`核验状态` 和 `核验记录`；只有 `数据模式=真实数据` 且核验来源为真实详情页/真实采集链路的数据，才可作为真实选品依据。
如果导出行显示 `数据模式=来源未知/需核验`，说明系统无法证明该行来自真实 1688 采集或真实详情核验，不能直接用于选品和铺货，必须重新采集或人工核验。
...
- 不绕过验证码，不模拟恶意高频访问。
- 触发滑块/验证码时系统会返回 `security_verification_required`，不会继续采集或生成导出；需要用户在真实浏览器中手动完成验证后重试。
- 使用真实账号扫码登录，不保存账号密码。
